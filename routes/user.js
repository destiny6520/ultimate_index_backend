				...
				if(result){
					jwt.verify(req.body.token, process.env.TOKENSECRET, function(error, decoded){
						if(decoded){
							...
							User.findOne({ email: req.body.email,session_id1: req.body.session_token }, function(error, loggedin){
								if(loggedin){
									...
									User.findOne({ email: req.body.email,session_id2: req.body.session_token }, function(error, loggedina){
										if(loggedina){
											var expiryUnixTime = decoded.exp * 1000;
											var issuedUnixTime = decoded.iat * 1000;
											const issueDate = new Date(issuedUnixTime).toLocaleString();
